...
const EventEdit = ({ title, event }) => {
  const [errors, setErorrs] = useState({});

  const [values, setValues] = useState({
    judul: event.judul,
    deskripsi: event.deskripsi,
    imgName1: event.imgName1,
    imgUrl1: event.imgUrl1,
    imgName2: event.imgName2,
  ...
  function handleSubmit() {
    axios
      .patch(`/super-admin/event/${event.id}`, values)
      .then((res) => {
...
EventEdit.layout = (page) => <SuperAdminTemplate children={page} />
